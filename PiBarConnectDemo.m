%% PiBarConnectDemo : Bar Chart of Proportion — 10-80 digits
...
% Get first 100 digits of pi (获取π的前100位数字)
Pi = getPi(100);
...
% Sample at every 10th digit (every 10 decimals) (每10位采样一次)
X = Ratio(:, 10:10:80)';

% Draw stacked bar chart (绘制堆叠柱状图)
barHdl = bar(X, 'stacked', 'BarWidth', 0.2);
...
% Extract Y-endpoints of each bar series (提取每个系列的Y端点)
...
for i = 1:length(barHdl)                           
    for j = 1:length(barHdl(1).YData) - 1          
        ...
        % Draw a filled polygon connecting the two bars (绘制连接两个条形的填充多边形)
        fill([j + 0.5 * barWidth, j + 1 - 0.5 * barWidth, j + 1 - 0.5 * barWidth, j + 0.5 * barWidth], ...
             [y1, y3, y4, y2], ...
             barHdl(i).FaceColor, 'FaceAlpha', 0.4, 'EdgeColor', 'none');
    end
end
...
ax.Title.String = 'Bar Chart of Proportion — 10-80 digits';
ax.Title.FontSize = 14;

% Add legend for digits 0-9 and decorate (添加数字0-9的图例并修饰)
