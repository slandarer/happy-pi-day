%% PiCircleRatio6Demo : Feynman Point of π —— 6-digit segments visualized as concentric circles
...
% Get the first 767 decimal digits, plus the leading 3 (获取前767位小数，加上整数部分的3)
Pi = [3, getPi(767)];

% Reference: digits 762-767 form the Feynman point (six 9s) (引用：第762-767位构成费曼点（六个9）)
...
% legend
for i = 1:10
    fill(x / 4 + 5.5 + (i - 5.5) * 0.32, ...
         y / 4 + 14.5, ...
         CM(i, :), 'EdgeColor', 'none');
    text(5.5 + (i - 5.5) * 0.32, 14.9, num2str(i-1), ...
         'Color', [1, 1, 1], 'FontSize', 9, 'FontName', 'Cambria', ...
         'HorizontalAlignment', 'center');
end

% Add title (添加标题)
text(5.5, -0.2, 'FEYNMAN POINT of \pi', ...
